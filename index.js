...
bot.on('ready', () =>{
    ...
    setInterval(function() {
        if (activNum === 0) { // Chechking on what activity it's on.
            ...
            bot.user.setActivity(`${bot.users.size}members`, {type: "WATCHING"});
            activNum = 2;
        }else if (activNum === 2) {
            bot.user.setActivity('s!help', {type: "LISTENING"});
            activNum = 0;
        }
    }, 3* 700); //Every 3 seconds. The * 1000 is here so the '3' is going to be converted from Milliseconds to Seconds
})
